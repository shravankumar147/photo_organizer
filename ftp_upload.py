...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        metavar="PATH",
        help="Source cloud-ready directory.",
    )
    parser.add_argument(
        "--trash",
        default=str(DEFAULT_TRASH),
        metavar="PATH",
        help="Local trash directory for successfully uploaded files.",
    )
    parser.add_argument(
        "--host",
        default=os.environ.get("FTP_HOST"),
        help="FTP host. Defaults to FTP_HOST.",
    )
    parser.add_argument(
        "--user",
        default=os.environ.get("FTP_USER"),
        help="FTP username. Defaults to FTP_USER.",
    )
    parser.add_argument(
        "--password",
        default=os.environ.get("FTP_PASSWORD"),
        help="FTP password. Defaults to FTP_PASSWORD.",
    )
    parser.add_argument(
        "--remote-root",
    ...
    parser.add_argument(
        "--port",
        default=int(os.environ.get("FTP_PORT", "21")),
        type=int,
        help="FTP port. Defaults to FTP_PORT or 21.",
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Show what would be uploaded without writing files or moving local files.",
    )
    return parser
